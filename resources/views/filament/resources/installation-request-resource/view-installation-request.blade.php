...
<div class="space-y-4 pb-10">
    ...
    {{-- ── Status Band (white card, same border as siblings) ───────────────── --}}
        ...
        <div class="flex flex-wrap items-center gap-x-6 gap-y-3">
            ...
            @if ($record->invoice_number)
                <div class="flex items-center gap-1.5">
                    ...
                    <span class="text-sm font-bold text-gray-800 dark:text-gray-100">B-{{ $record->invoice_number }}</span>
                </div>
                <div class="h-6 w-px bg-gray-200 dark:bg-gray-600 hidden sm:block"></div>
            @endif

            <div class="flex items-center gap-1.5">
                <span class="text-xs font-medium text-gray-400 dark:text-gray-500">{{ __('Product') }}</span>
                <span class="text-sm font-bold text-gray-800 dark:text-gray-100">{{ $record->product_type }}</span>
            </div>

            <div class="h-6 w-px bg-gray-200 dark:bg-gray-600 hidden sm:block"></div>

            <div class="flex items-center gap-1.5">
                <span class="text-xs font-medium text-gray-400 dark:text-gray-500">{{ __('Quantity') }}</span>
                <span class="text-sm font-bold text-gray-800 dark:text-gray-100">{{ $record->quantity }} unit{{ $record->quantity != 1 ? 's' : '' }}</span>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div wire:ignore class="bg-white dark:bg-gray-800 rounded-xl shadow overflow-hidden">
                ...
                @if ($hasLocation)
                    <div id="installation-map" style="height: 360px; position: relative; z-index: 0;"></div>
                @else
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-5">
                ...
                <div class="flex items-center gap-2 mb-5">
                    ...
                    <span class="font-semibold text-gray-900 dark:text-white">{{ __('Installation Details') }}</span>
                </div>

                <div class="grid grid-cols-3 gap-3 mb-5">

                    {{-- Product --}}
                    <div class="rounded-xl bg-blue-50 dark:bg-blue-900/20 p-4 border border-blue-100 dark:border-blue-800/40">
                        <div class="text-xs font-semibold uppercase tracking-wider text-blue-500 dark:text-blue-400 mb-2">{{ __('Product') }}</div>
                        <div class="font-bold text-gray-900 dark:text-white text-sm leading-snug">{{ $record->product_type }}</div>
                        @if ($record->invoice_number)
                            <div class="mt-2.5 inline-flex items-center gap-1 text-xs font-semibold text-blue-700 dark:text-blue-300 bg-blue-100 dark:bg-blue-800/40 px-2 py-0.5 rounded-md">
                                <span class="opacity-60 text-[10px]">INV</span>
                                B-{{ $record->invoice_number }}
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="rounded-xl bg-gray-50 dark:bg-gray-700/50 p-4 border border-gray-100 dark:border-gray-700">
                        <div class="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-2">{{ __('Quantity') }}</div>
                        <div class="text-4xl font-black text-gray-900 dark:text-white leading-none">{{ $record->quantity }}</div>
                        <div class="text-xs font-medium text-gray-400 dark:text-gray-500 mt-1.5">unit{{ $record->quantity != 1 ? 's' : '' }}</div>
                    </div>

                    {{-- Site Status --}}
                    @if ($record->is_site_ready)
                        <div class="rounded-xl bg-green-50 dark:bg-green-900/20 p-4 border border-green-100 dark:border-green-800/40">
                            <div class="text-xs font-semibold uppercase tracking-wider text-green-600 dark:text-green-400 mb-2">{{ __('Site Status') }}</div>
                            <div class="flex items-center gap-1.5">
                                ...
                                <span class="font-bold text-green-700 dark:text-green-400 text-sm">{{ __('Site Ready') }}</span>
                            </div>
                        </div>
                    @else
                        <div class="rounded-xl bg-orange-50 dark:bg-orange-900/20 p-4 border border-orange-100 dark:border-orange-800/40">
                            <div class="text-xs font-semibold uppercase tracking-wider text-orange-500 dark:text-orange-400 mb-2">{{ __('Site Status') }}</div>
                            <div class="flex items-center gap-1.5">
                                <div class="w-6 h-6 rounded-full bg-orange-400 flex items-center justify-center flex-shrink-0">
                                    <svg class="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"/>
                                    </svg>
                                </div>
                                <span class="font-bold text-orange-700 dark:text-orange-400 text-sm">{{ __('Not Ready') }}</span>
                            </div>
                        </div>
                    @endif
        ...
        <div class="space-y-4">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center gap-3 p-3 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800/40">
                        <div class="w-9 h-9 rounded-lg bg-blue-100 dark:bg-blue-900/40 flex items-center justify-center flex-shrink-0">
                            <svg class="w-4.5 h-4.5 text-blue-500 dark:text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="width:18px;height:18px;">
            ...
            @if ($activities->count())
                @php
                    ...
                    // Build a friendly event object for each activity
                    $buildEvent = function($activity) use ($statusLabels) {
                        ...
                        if ($desc === 'created') {
                            return [
                                'phrase' => __('Installation request was created'),
                                'sub'    => __('New request submitted and waiting for assignment'),
                                'icon'   => 'created',
                                'color'  => 'blue',
                            ];
                        }
                        ...
                        if ($desc === 'updated') {
                            ...
                            // Status + technician together
                            if ($hasStatus && $hasTech) {
                                $newStatus = $statusLabels[$attrs['status']] ?? $attrs['status'];
                                return [
                                    'phrase' => __('Technician assigned and status set to ":status"', ['status' => $newStatus]),
                                    'sub'    => null,
                                    'icon'   => 'assign',
                                    'color'  => 'purple',
                                ];
                            ...
                            if ($hasStatus) {
                                ...
                                return [
                                    'phrase' => __('Status updated to ":status"', ['status' => $newStatus]),
                                    'sub'    => $sub,
                                    'icon'   => $icon,
                                    'color'  => $color,
                                ];
                            ...
                            if ($hasTech) {
                                ...
                                return [
                                    'phrase' => $wasReassign ? __('Technician was reassigned') : __('Technician was assigned'),
                                    'sub'    => null,
                                    'icon'   => 'assign',
                                    'color'  => 'purple',
                                ];
...
(function () {
    ...
    function boot() {
        var el = document.getElementById('installation-map');
